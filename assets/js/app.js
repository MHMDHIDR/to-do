...
function makeAction(e) {
  ...
  if(targetElem.classList.contains('trash-icon')) {
    ...
    deleteTodo(targetElem.parentNode.previousElementSibling.getAttribute('data-id'));
  }
  ...
  if (targetElem.classList.contains(UNCHECK)) {
    toggleTodo(targetElem.nextElementSibling.getAttribute('data-id'))
...
const renderTodos = (todos) => {
  ...
  todos.forEach(todo => {
    ...
    const taskTemplate = 
    `<li class="task">
      <span class="action-btn circle ${todo.completed?'checked':''}"></span>
      <p class="todo" data-id="${todo.id}">${todo.name}</p>
      <div class="options">
        <!-- edit icon -->
...
const addTask = (e) => {
  ...
  // make todo object to store the value and it's information
  const toDo = {
    id: Date.now(),
  ...
  // insert task into todos array
  todos.push(toDo);

  // Render todos
  addToLocalStorage(todos);

  // empty input and set foucs again
  inputTask.value = "";
  inputTask.focus();
}

/* === Adding todos to localstorage then displaying (rendering) using different func === */
const addToLocalStorage = (todos) => {
  // set todos array as string inside localStroage
  localStorage.setItem('todos', JSON.stringify(todos));

  // getting/rendering stuff out from the localStorage
  renderTodos(todos);
}

/* === get todos from localStorage === */
const getFromLocalStorage = () => {
  // check if we've todos list in localStorage
  todosList = localStorage.getItem('todos');

  if(todosList) {
    // convert items to array and insert it to todos array
...
const toggleTodo = (id) => {
  todos.forEach(todo => {
    ...
    todo.completed = !todo.completed; // toggle it true or false
